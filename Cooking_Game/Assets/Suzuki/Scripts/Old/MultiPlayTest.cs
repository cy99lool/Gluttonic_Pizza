...
public class MultiPlayTest : MonoBehaviour
{
    ...
    {
        port = opponentPort;
        byte[] message = UDPMessage.Ack.ToByteTest();
    ...
    {
        //なるべくメッセージごと１関数で実装する
        UDPMessage type = unit.message.ToUDPMessageTest();
            ...
            case UDPMessage.Ack:
                {
                    ...
                    print("他の人から接続がありました");
                    byte[] message = UDPMessage.AckComplete.ToByteTest();
    ...
    void BroadcastStatus()
    {
        ...
        //データを作る
        byte[] message = UDPMessage.PosUpdate.ToByteTest();
...
{
    public static byte[] ToByteTest(this UDPMessage udpm)
    {
        return BitConverter.GetBytes((int)udpm);
    }

    public static UDPMessage ToUDPMessageTest(this byte[] b, int startIndex = 0)
    {
        int number = BitConverter.ToInt32(b, startIndex);
        return (UDPMessage)Enum.ToObject(typeof(UDPMessage), number);
    }
    public static byte[] ToByteTest(this Vector3 v)
    {
    ...
    public static Vector3 ToVector3Test(this byte[] b, int startIndex)
    {
    ...
    public static int IndexOfPortTest(this List<IPEndPoint> eps, int targetPort)
    {
        int index = -1;
